 ...
 *
 *   $Id: HistoryEditorDialogBox.cs,v 1.11 2007-07-23 09:03:44 smithydll Exp $
 *
 *
 ***************************************************************************/
...
namespace ModStudio
{
	...
	public class HistoryEditorDialogBox : System.Windows.Forms.Form
	{
        ...
        private ComboBox ModVersionStage;
		/// <summary>
		...
		private void buttonOk_Click(object sender, System.EventArgs e)
		{
			...
			entry.Version.Minor = (int)MODVersionMinor.Value;
			entry.Version.Revision = (int)MODVersionRevision.Value;
            if (ModVersionStage.Text == VersionStage.Alpha.ToString())
            {
                entry.Version.Stage = VersionStage.Alpha;
            }
            else if (ModVersionStage.Text == VersionStage.Beta.ToString())
            {
                entry.Version.Stage = VersionStage.Beta;
            }
            else if (ModVersionStage.Text == VersionStage.ReleaseCandidate.ToString())
            {
                entry.Version.Stage = VersionStage.ReleaseCandidate;
            }
            else
            {
                entry.Version.Stage = VersionStage.Stable;
            }
			entry.Date = MODHistorydtp.Value;
			if (MODVersionRelease.Text.Length > 0)
			{
